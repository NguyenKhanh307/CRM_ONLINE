...
import { EMPTY_FORM, REQUEST_QUOTE_POINTS } from '../config/trackingDemoConfig';
...
// bước cuối — form liên hệ: gửi kèm giỏ sản phẩm đã chọn, biến khách ẩn danh thành tiềm năng
// có tên/email/sđt + các dòng lead_items. Đây là form khách tự điền trên website, KHÔNG phải
// thao tác thêm/sửa trong CRM, nên cố ý chỉ báo lỗi đỏ dưới ô — không có popup xác nhận
export const LeadContactForm = ({ busy, itemCount, onSubmit }: Props) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const errs = collectErrors({
            ...
            items: itemCount === 0 ? 'Vui lòng chọn ít nhất 1 sản phẩm ở trên' : null,
        });
        setErrors(errs);
    ...
    return (
        <form ref={formRef} onSubmit={handleSubmit} noValidate className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            ...
            <p className="ml-8 mb-3 text-sm text-gray-500">
                Gửi yêu cầu báo giá (+{REQUEST_QUOTE_POINTS} điểm): khách ẩn danh trở thành tiềm năng có tên,
                email, số điện thoại thật và danh sách sản phẩm quan tâm trong CRM.
            </p>
            ...
            <div className="ml-8 space-y-3">
                ...
                >
                    Gửi yêu cầu báo giá (+{REQUEST_QUOTE_POINTS} điểm)
                </button>
            </div>
        </form>
    );
