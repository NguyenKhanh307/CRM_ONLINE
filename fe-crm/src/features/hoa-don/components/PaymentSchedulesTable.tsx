    ...
    return (
        <tr>
            ...
            <td className="px-2 py-1 border-b border-gray-100">
                <input type="number" min={0} className={`${inp} text-right`} value={form.amount ?? ''} onChange={(e) => setForm((f) => ({ ...f, amount: e.target.value ? +e.target.value : null }))} />
            </td>
            <td className="px-2 py-1 border-b border-gray-100">
                <input type="number" min={0} className={`${inp} text-right`} value={form.paidAmount ?? ''} onChange={(e) => setForm((f) => ({ ...f, paidAmount: e.target.value ? +e.target.value : null }))} />
            </td>
            <td className="px-2 py-1 border-b border-gray-100">
            ...
            <td className="px-2 py-1 border-b border-gray-100 text-center whitespace-nowrap">
                <button type="button" title="Lưu đợt" disabled={update.isPending}
                    ...
                    onClick={() => update.mutate({ id: s.id, payload: form })}>
                    <FiSave size={13} />
                </button>
                ...
                <table className="w-full border-collapse" style={{ minWidth: 620 }}>
                    ...
                    <tbody>
                        ...
                        <tr className="bg-gray-50">
                            ...
                            <td className="px-2 py-1"><input type="number" min={0} className={`${inp} text-right`} value={draft.amount ?? ''} onChange={(e) => setDraft((f) => ({ ...f, amount: e.target.value ? +e.target.value : null }))} /></td>
                            <td className="px-2 py-1"><input type="number" min={0} className={`${inp} text-right`} value={draft.paidAmount ?? ''} onChange={(e) => setDraft((f) => ({ ...f, paidAmount: e.target.value ? +e.target.value : null }))} /></td>
                            ...
                            <td className="px-2 py-1 text-center">
                                <button type="button" title="Thêm đợt" disabled={add.isPending || !draft.amount}
