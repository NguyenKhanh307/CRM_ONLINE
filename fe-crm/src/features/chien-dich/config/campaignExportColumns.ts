...
export const campaignExportColumns: ExportColumn<CampaignResult>[] = [
    ...
    { key: 'actualCost', label: 'Chi phí thực tế', format: r => r.actualCost ?? '' },
    { key: 'targetSize', label: 'Quy mô mục tiêu', format: r => r.targetSize ?? '' },
    { key: 'expectedRevenue', label: 'Doanh thu kỳ vọng', format: r => r.expectedRevenue ?? '' },
