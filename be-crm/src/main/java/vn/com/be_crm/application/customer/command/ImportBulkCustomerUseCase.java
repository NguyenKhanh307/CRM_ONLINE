...
// nhập hàng loạt Customer từ file Excel/CSV — validate TOÀN BỘ trước (pass 1, không đụng DB), chỉ
...
public class ImportBulkCustomerUseCase {
    ...
    public ImportBulkCustomerUseCase(ICustomerRepository repo, ITransactionRunner txRunner) {
        this.repo = repo;
    ...
    public ImportBulkResult execute(ImportBulkCustomerCommand cmd) {
        ...
        for (int i = 0; i < cmd.rows().size(); i++) {
            ...
            if (err == null) err = ImportValidators.taxCodeError(row.taxCode());
            if (err == null && !isValidEnumOrBlank(CustomerType.class, row.type())) {
                err = "Trường \"Loại khách hàng\" giá trị không hợp lệ: \"" + row.type() + "\"";
            }
            if (err == null && !isValidEnumOrBlank(CustomerStatus.class, row.status())) {
