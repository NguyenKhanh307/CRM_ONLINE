 ...
 * - Nếu đang trong một Unit of Work ({@link SessionHolder} có session) → dùng
 * session ambient,
 * KHÔNG tự commit/close (để {@link HibernateTransactionRunner} bên ngoài lo).
 * - Nếu gọi đơn lẻ (ngoài Unit of Work) → mở session + transaction riêng,
 * commit + close như cũ.
 * Nhờ đó nhiều lệnh ghi trong một use case đa bước cùng commit/rollback, nhưng
 * khi gọi lẻ vẫn
...
public final class TxSupport {
    ...
    /**
     * Thao tác ĐỌC. Trong Unit of Work → dùng session ambient (thấy được thay đổi
     * chưa commit
     * của cùng transaction); ngoài Unit of Work → mở session tạm (không
     * transaction) rồi đóng.
     *
     * @param sf   SessionFactory
