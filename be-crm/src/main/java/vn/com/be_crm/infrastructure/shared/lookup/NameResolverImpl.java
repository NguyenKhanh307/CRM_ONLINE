package vn.com.be_crm.infrastructure.shared.lookup;
 ...
 *
 * <p>Mỗi method chạy một native query {@code SELECT id, <col> FROM <table> WHERE id IN (:ids)}
 * — <b>không</b> lọc {@code deleted_at}/{@code is_purged} nên tên vẫn resolve cho bản ghi đã xóa.
 */
@Repository
public class NameResolverImpl implements INameResolver {
    ...
    public NameResolverImpl(SessionFactory sf) { this.sf = sf; }

    /** {@inheritDoc} */
    @Override public Map<Long, String> users(Collection<Long> ids) { return resolve("users", "full_name", ids); }
    /** {@inheritDoc} */
    @Override public Map<Long, String> customers(Collection<Long> ids) { return resolve("customers", "name", ids); }
    /** {@inheritDoc} */
    @Override public Map<Long, String> contacts(Collection<Long> ids) { return resolve("contacts", "full_name", ids); }
    /** {@inheritDoc} */
    @Override public Map<Long, String> campaigns(Collection<Long> ids) { return resolve("campaigns", "name", ids); }
    /** {@inheritDoc} */
    @Override public Map<Long, String> opportunities(Collection<Long> ids) { return resolve("opportunities", "name", ids); }
    /** {@inheritDoc} */
    ...
    /** {@inheritDoc} */
    @Override public Map<Long, String> productCategories(Collection<Long> ids) { return resolve("product_categories", "name", ids); }

    /**
     * Chạy native query tra id→tên cho một bảng.
     * Tham số bảng/cột là hằng nội bộ (không nhận từ input người dùng) nên không có rủi ro SQL injection.
    ...
    private Map<Long, String> resolve(String table, String nameCol, Collection<Long> ids) {
        // Trả HashMap rỗng (mutable, get(null)-safe) thay Map.of() — Map bất biến ném NPE khi get(null).
        ...
        if (distinct.isEmpty()) return new HashMap<>();
