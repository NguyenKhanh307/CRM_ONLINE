package vn.com.be_crm.infrastructure.shared.ai;
 ...
 * Cùng khuôn với {@link GeminiAiServiceImpl} — dùng RestClient của spring-web, không thêm thư viện.
 * <p>Chỉ chạy 1 lần cho mỗi câu hỏi của người dùng (vài chục request/ngày), nên không bao giờ
 * chạm trần hạn mức. Phần tốn hạn mức là đánh chỉ mục dữ liệu — nằm ở {@code tools/indexer/}.
 */
@Component
public class GeminiEmbeddingServiceImpl implements IEmbeddingService {
    ...
    private float[] extractValues(Map<String, Object> res) {
        ...
        // Số chiều lệch nghĩa là cấu hình backend khác indexer -> mọi kết quả tìm kiếm sẽ sai.
        ...
        for (int i = 0; i < out.length; i++) out[i] = values.get(i).floatValue();
        return out;
    }

    /**
     * Chuẩn hóa L2 (độ dài vector = 1).
     * <p>🚨 BẮT BUỘC: {@code gemini-embedding-001} chỉ tự chuẩn hóa ở 3072 chiều; dùng
     * {@code outputDimensionality} nhỏ hơn thì Google yêu cầu client tự normalize.
     * Công thức này phải giống hệt {@code tools/indexer/embed.py::normalize} — lệch nhau là
