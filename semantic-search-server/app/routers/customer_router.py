...
def upsert_customers(body: UpsertRequest):
    client = get_client()
    points = []
    ...
    client.upsert(collection_name=settings.customer_collection, points=points)
    return {"upserted": len(points)}


@router.post("/search", response_model=SearchResponse, dependencies=[Depends(verify_api_key)])
def search_customers(body: SearchRequest):
    client = get_client()
    hits = client.search(
        collection_name=settings.customer_collection,
...
def delete_customers(body: DeleteRequest):
    client = get_client()
    client.delete(
        collection_name=settings.customer_collection,
